...
#
#      AUTHOR: @KodoDrive - Telegram
#     VERSION: 4.0.2 (Исправлена ошибка DNS-разрешения при получении SSL)
#     CREATED: $(date)
#
# =====================================================================================
...
║    ╚═══╝  ╚═╝     ╚═╝  ╚═══╝    ╚═╝╚═╝  ╚═══╝╚══════╝   ╚═╝   ║
║                                                               ║
║        Enhanced VPN Server Auto Installer v4.0.2             ║
...
cleanup_on_error() {
    ...
    # Не восстанавливаем DNS, так как он мог быть причиной сбоя
...
fix_local_dns() {
    ...
    # Удаляем символическую ссылку, если она существует
    if [ -L /etc/resolv.conf ]; then
        rm -f /etc/resolv.conf
    fi
    # Записываем публичные DNS серверы для обеспечения работы установки
    cat > /etc/resolv.conf << EOF
# Generated by vpn-installer for stable DNS resolution
nameserver 1.1.1.1
nameserver 8.8.8.8
EOF
    log_info "Локальный DNS настроен на использование 1.1.1.1 и 8.8.8.8 ✅"
}
...
check_dns_resolution() {
    ...
    local resolved_ip
    # Используем dig с указанием внешнего резолвера, чтобы обойти возможные локальные кэши
    ...
    if [[ -z "$resolved_ip" ]]; then
        ...
        log_info "DNS запись домена корректна, $DOMAIN указывает на $SERVER_IP ✅"
    fi
}
...
install_adguard() {
    ...
    # Создаем временный конфиг для генерации хеша пароля
...
EOF
    ...
    # Опция -s install сама обработает конфиг и сохранит хеш пароля
    /opt/AdGuardHome/AdGuardHome -s install >/dev/null

    # Теперь, когда сервис установлен, мы можем записать наш рабочий конфиг
    # AdGuard при запуске прочитает его и применит
    cat > /opt/AdGuardHome/AdGuardHome.yaml << EOF
bind_host: 127.0.0.1
bind_port: $ADGUARD_PORT
auth_attempts: 5
# Пароль уже хеширован и сохранен сервисом при установке
language: ru
dns:
  bind_hosts:
...
certbot delete --cert-name $DOMAIN --non-interactive || echo "Certbot certs for $DOMAIN not found."
echo "Removing web root and logs..."
rm -rf /var/www/html $LOG_FILE $STATE_FILE
...
main() {
    ...
    install_dependencies
    stop_conflicting_services
    fix_local_dns # <-- Важное исправление
    check_dns_resolution
    configure_firewall
    setup_ssl
    install_3x_ui
